...
import { Cpu, Sparkles, Wand2, Globe, Zap, ArrowRight, CheckCircle2 } from "lucide-react";
import Image from "next/image";

export const metadata: Metadata = {
  title: "DP AI Studios | High-Velocity AI Media & CGI Production India",
  description: "The next generation of media production. DP AI Studios provides generative AI video, photorealistic avatars, and AI-CGI virtual production for global brands.",
  keywords: ["AI video production company India", "DP AI Studios", "generative AI media Kerala", "AI CGI virtual production India"],
};
...
export default function AIStudiosPage() {
  return (
    <main className="min-h-screen bg-obsidian text-white pt-40 px-6 md:px-12 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-0 w-[500px] h-[500px] bg-blue-500/10 blur-[180px] rounded-full pointer-events-none" />
      ...
      <div className="container mx-auto max-w-6xl relative z-10">
        <header className="mb-40 text-center flex flex-col items-center">
           ...
           <h1 className="text-7xl md:text-[12rem] font-black tracking-tighter uppercase mb-12 leading-[0.8] text-white">
             DP AI <span className="text-primary-accent italic">Studios_</span>
           </h1>
        ...
        <div className="grid md:grid-cols-2 gap-12 mb-40">
           {[
             ...
             <div 
               key={pillar.title} 
               onClick={() => window.location.href = pillar.href}
             ...
             >
                <div className={`mb-10 h-16 w-16 rounded-2xl bg-gradient-to-tr ${pillar.color} flex items-center justify-center text-black group-hover:scale-110 transition-transform`}>
                   <pillar.icon size={32} />
                </div>
                <h3 className="text-4xl font-black uppercase tracking-tight text-white mb-6 group-hover:text-primary-accent transition-colors">{pillar.title}</h3>
                <p className="text-lg text-zinc-500 font-light leading-relaxed mb-10">{pillar.desc}</p>
                <div className="flex items-center gap-4 text-xs font-mono tracking-widest uppercase text-zinc-600 group-hover:text-white transition-colors">
                   Enter Node <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform" />
                </div>
        ...
        <section className="mb-40 grid lg:grid-cols-2 gap-20 items-center">
           <div className="space-y-12">
              <h2 className="text-4xl md:text-6xl font-black uppercase tracking-widest leading-none">Why Choose <br /><span className="text-primary-accent italic">AI Velocity?</span></h2>
              <ul className="space-y-10">
                {[
                  { t: "10x Production Speed", d: "Production cycles reduced from months to days using proprietary neural pipelines." },
                  { t: "Cost-Effective Scaling", d: "Generate hundreds of visual variants for the cost of a single traditional project." },
                  { t: "Global Reach (Multilingual)", d: "Instantly translate and dub your video content into over 50 regional and global languages." }
                ].map(item => (
        ...
        <div className="py-40 text-center border-t border-white/5 mt-40">
           <h2 className="text-5xl md:text-[10rem] font-black uppercase tracking-tighter mb-16 text-white leading-none">
             Future-proof <br /> <span className="text-primary-accent italic">your brand.</span>
           </h2>
           <button 
             onClick={() => window.location.href = '/contact?subject=AI Studio Project enquity'}
